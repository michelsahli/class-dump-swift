...
std::string signatureFromMangledSymbolString(const char *manl) {
	std::string command = "xcrun swift-demangle ";

	if(isOutputCompact)         command += "-compact '";
	else if(isOutputSimplified) command += "-simplified '";
	else                        command += "'";
	...
	if(!isOutputCompact) {
		...
		size_t pos = ret.find(searchStr) + strlen(searchStr);
		ret.erase(0, pos);
	}
